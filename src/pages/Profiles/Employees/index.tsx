...
const EmployeeManagement: React.FC = () => {
    ...
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedIds(employees.map((e: EmployeeProfile) => e.userId));
        } else {
            setSelectedIds([]);
        }
    };
    ...
    const handleBulkDelete = async () => {
        ...
        const confirmed = await confirm({
            ...
            message: `Are you sure you want to permanently delete ${selectedIds.length} selected employee profiles? This action cannot be undone.`,
        ...
        if (confirmed) {
            try {
                ...
                showSuccess(`Successfully removed ${selectedIds.length} employee profiles.`);
                setSelectedIds([]);
            } catch (e) {
                showError(e, "Failed to remove some employees");
            }
        }
    };
                ...
                {selectedIds.length > 0 && (
                    <div className="flex items-center justify-between p-4 bg-brand-50 border border-brand-100 rounded-2xl dark:bg-brand-500/10 dark:border-brand-500/20 animate-in slide-in-from-top-2 duration-300">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-semibold text-brand-700 dark:text-brand-400">Employees Selected</p>
                        </div>
                        ...
                        <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                            {isEmployeesLoading ? (
                                ...
                                employees.map((employee: EmployeeProfile) => (
                                    <TableRow key={employee.id} className={`group hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors ${selectedIds.includes(employee.userId) ? 'bg-brand-50/30 dark:bg-brand-500/5' : ''}`}>
                                        <TableCell className="px-5 py-4">
                                            <div className="flex items-center">
                                                <input 
                                                    ...
                                                    onChange={() => handleSelectRow(employee.userId)}
                                                />
                                            </div>
                                        </TableCell>
                                        <TableCell className="px-5 py-4">
