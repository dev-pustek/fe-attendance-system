...
const ScheduleBuilderTab = () => {
        ...
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-gray-200 dark:border-white/10 rounded-2xl">
                ...
                <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-4">
                    There are no subjects scheduled for this class yet.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="h-full flex flex-col gap-4 w-full max-w-full overflow-hidden p-4">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={async () => {
                            try {
                                if (!classId) return;
                                const blob = await academicService.printClassSchedule(classId);
                                const filename = `schedule-class-${classId}.pdf`;
                    ...
                    <button
                        onClick={async () => {
                            try {
                                if (!classId) return;
                                const blob = await academicService.exportExcelClassSchedule(classId);
                                const filename = `schedule-class-${classId}.xlsx`;
