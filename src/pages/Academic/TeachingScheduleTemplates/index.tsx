...
import { useTeachingScheduleTemplates, useClasses } from "../../../api/hooks/useAcademic";
...
const DraggableSubject: React.FC<DraggableItemProps & {
    unitUsage?: number;
    unitTotal?: number;
    ...
    const subject = isClassSubject ? (data as ClassSubject).subject : (data as TeacherSubject).subject;
    // Extract all assigned teachers for ClassSubject
    const assignedTeachers = isClassSubject
        ? (data as ClassSubject).teachingAssignments?.map(a => a.teacher).filter(Boolean) || []
    ...
    const associatedClasses = !isClassSubject 
        ? ((data as TeacherSubject).subject as unknown as Subject & { classSubjects: ClassSubject[] })?.classSubjects?.map((cs: ClassSubject) => cs.class?.name).filter(Boolean)
        : null;
        ...
        >
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex justify-between items-start">
                    <span className="font-bold text-sm leading-tight line-clamp-2" style={{ color: textColor }}>
                        {subjectName}
                    </span>
                 ...
                 {showJp && (
                    <div className="mt-1 w-full flex flex-col gap-0.5">
                        <div className="flex justify-between items-center text-[10px] font-bold opacity-80" style={{ color: textColor }}>
                           <span>{unitUsage} / {unitTotal} JP</span>
                   ...
                   <div className="grid grid-cols-2 gap-1.5">
                       {assignedTeachers.length > 0 ? (
                           assignedTeachers.map((t, i) => (
                               <div key={t?.id || i} className="flex items-center gap-1.5 overflow-hidden" title={t?.name}>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-1">
                    {className && (
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-white/60 border border-black/5" style={{ color: textColor }}>
                            {className}
                        </span>
                    )}
                    ...
                    {associatedClasses && associatedClasses.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                            {associatedClasses.slice(0, 3).map((cls, i) => (
                                <span key={i} className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-white/40 border border-black/5" style={{ color: textColor }}>
                                    {cls}
                                </span>
                            ))}
                            {associatedClasses.length > 3 && (
                                <span className="text-[9px] opacity-70 px-1" style={{ color: textColor }}>+{associatedClasses.length - 3} more</span>
                            )}
                        </div>
                    )}

                    {!isClassSubject && (!associatedClasses || associatedClasses.length === 0) && (
                         <span className="text-[10px] italic opacity-60 px-1" style={{ color: textColor }}>
                            Any Class
                        </span>
                    )}
                </div>
...
const AvailabilityTimeline: React.FC<{
    ...
    const effectiveRule = rule && rule.isActive ? rule : {
        startTime: "07:00",
        endTime: "16:00",
        breaks: [],
        isActive: true
    ...
    const getPos = (time: string) => {
        ...
        return Math.max(0, Math.min(100, ((mins - dayStart) / totalDuration) * 100));
    };

    // --- GAP CALCULATION ---
    const allOccupied = [
        ...(effectiveRule.breaks || []).map(b => ({ start: toMinutes(b.startTime), end: toMinutes(b.endTime), type: 'break' })),
        ...otherSessions.map(s => ({ start: toMinutes(s.startTime), end: toMinutes(s.endTime), type: 'session' }))
    ...
    for (const block of checkPoints) {
        if (block.start > pointer) {
            ...
            if (duration >= 45) { // Minimum 45 mins to be suggested
                    ...
                    {otherSessions.map((s, i) => {
                        ...
                        return (
                            <div 
                                ...
                                style={{ left: `${startPos}%`, width: `${Math.max(0.5, endPos - startPos)}%` }}
                            />
                        );
                    })}
                ...
                <div className="space-y-1.5">
                    <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-400">
                        <PlusIcon className="size-3" />
                         <span>Smart Suggestions (Found {availableSlots.length} available slots)</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {availableSlots.slice(0, 4).map((slot, i) => (
                            <button
                                key={i}
                                type="button"
                                onClick={() => onSelectTime(formatTime(slot.start), formatTime(Math.min(slot.start + 90, slot.end)))} // Suggest 2 JP by default
                            ...
                            >
                                ...
                                {slot.end - slot.start > 90 && <span className="text-[9px] opacity-60 ml-1">(of {Math.round((slot.end - slot.start))}m)</span>}
                            </button>
                        ))}
                    </div>
                </div>
...
const TeachingScheduleTemplates: React.FC = () => {
  ...
  const [selectedTeacher, setSelectedTeacher] = useState<{ label: string; value: string; subLabel?: string } | null>(null);
  
  // Class Filter (for effective rules visualization and sidebar filtering in Teacher Mode)
  const [selectedSidebarClassFilter, setSelectedSidebarClassFilter] = useState<string>("");
  ...
  const [droppedSubject, setDroppedSubject] = useState<TeacherSubject | null>(null);
  ...
  const classOptions: { label: string; value: string }[] = useMemo(() => classesData?.data.map((c: Class) => ({
    label: c.name,
    value: String(c.id)
  })) || [], [classesData]);
  ...
  const { data: classSubjectsResponse, isLoading: isLoadingSubjects } = useQuery({
      queryKey: ['academic', 'class-subjects', viewMode, selectedViewClass, subjectSearch],
      queryFn: () => academicService.getClassSubjects({
          classId: (viewMode === 'subject' && selectedViewClass) ? selectedViewClass : undefined,
  ...
  const { data: response, createMutation, updateMutation, deleteMutation, isLoading: isLoadingTemplates } = useTeachingScheduleTemplates({
    classId: viewMode === 'subject' ? (selectedViewClass || undefined) : undefined, 
  ...
  const minutesPerUnit = useMemo(() => policyData?.data?.minutesPerUnit || 45, [policyData]);
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<TeachingScheduleTemplate | null>(null);

  // Form Data
  const handleOpenModal = (day?: string, template?: TeachingScheduleTemplate, overrideClassSubject?: ClassSubject) => {
    ...
    if (template) {
        ...
        if (template.defaultTeacher) {
            setTeacherOptions(prev => {
                ...
                const teacherId = String(template.defaultTeacher?.id || template.defaultTeacherId);
                const teacherName = template.defaultTeacher?.name || "Unknown Teacher";
        ...
        setFormData({
            ...
            defaultTeacherId: template.defaultTeacherId ? String(template.defaultTeacherId) : "",
        ...
        if (viewMode === 'subject' && targetSubject) {
             ...
             if (!initialTeacherId && targetSubject.teachingAssignments && targetSubject.teachingAssignments.length > 0) {
                 const primary = targetSubject.teachingAssignments.find(ta => ta.role === 'primary');
                 const target = primary || targetSubject.teachingAssignments[0];
                 
                 if (target && target.teacher) {
                     initialTeacherId = String(target.teacher.public_id || target.teacher.id);
        ...
        if (effectiveRules) {
            const rule = effectiveRules[defaultDay] || effectiveRules[defaultDay.toUpperCase()];
  ...
  const [isSearchingClassSubjects, setIsSearchingClassSubjects] = useState(false);
  const [classSubjectOptions, setClassSubjectOptions] = useState<{ label: string; value: string }[]>([]);
  
  const searchClassSubjects = useCallback(async (term: string) => {
    ...
    try {
      ...
      setClassSubjectOptions(resp.data.map(cs => ({
  ...
  const effectiveClassId = useMemo(() => {
        ...
        if (viewMode === 'teacher' && formData.classSubjectId) {
            ...
            const found = availableClassSubjects.find(cs => String(cs.id) === String(formData.classSubjectId));
            if (found) return found.classId || found.class?.id;
        }

        return null;
  }, [viewMode, selectedViewClass, formData.classSubjectId, availableClassSubjects, selectedSidebarClassFilter]);


  const { data: rulesResponse } = useQuery({
      queryKey: ['rules', 'effective', effectiveClassId],
      queryFn: () => effectiveClassId ? ruleService.getEffectiveScheduleRules({ classId: effectiveClassId }) : null,
  ...
  const handleDayChange = (day: string) => {
      ...
      setFormData(prev => ({ ...prev, dayOfWeek: day, startTime: newStart, endTime: newEnd }));
  };


  // Fetch Teacher Subjects (for sidebar)
  const { data: teacherSubjectsResponse, isLoading: isLoadingTeacherSubjects } = useQuery({
  ...
  const { data: teacherWorkloadResponse } = useQuery({
      queryKey: ['academic', 'workload-contracts', 'teacher-active', selectedTeacher?.value],
      queryFn: () => academicService.getWorkloadContracts({
          teacherId: selectedTeacher?.value,
  ...
  const teacherWorkload = useMemo(() => teacherWorkloadResponse?.data?.[0], [teacherWorkloadResponse]);

  const teacherSubjects = useMemo(() => teacherSubjectsResponse?.data || [], [teacherSubjectsResponse]);
  ...
  const searchTeachers = useCallback(async (term: string) => {
    ...
    if (viewMode === 'subject' && selectedClassSubject) {
        ...
        const filtered = assignments
            .map(a => a.teacher)
            .filter(t => t && t.name.toLowerCase().includes(term.toLowerCase()));
        
        setTeacherOptions(filtered.map(t => ({
             label: t!.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (effectiveRules) {
        ...
        const rule = effectiveRules[dayKey] || 
                     effectiveRules[dayKey.charAt(0).toUpperCase() + dayKey.slice(1).toLowerCase()] ||
                     effectiveRules[dayKey.toUpperCase()];

        if (!rule || !rule.isActive) {
            showError(`Scheduling on ${formData.dayOfWeek} is not allowed by the effective schedule rules.`);
            return;
        }
        ...
        if (selectedStart < ruleStart || selectedEnd > ruleEnd) {
            showError(`Invalid Time. Session time must be within ${ruleStart} - ${ruleEnd}.`);
            return;
        }
    }
    ...
    const calculatedUnits = durationMins > 0 ? Number((durationMins / minutesPerUnit).toFixed(2)) : 0;
  ...
  const handleMoveSession = async (templateId: number | string, newDay: string, newStartTime: string, newEndTime: string) => {
        try {
            await updateMutation.mutateAsync({
                id: templateId,
                data: {
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <PageMeta title="Teaching Schedule Templates | Sistem Akademik" description="Manage weekly class schedules and teacher assignments." />
                           ...
                           {viewMode === 'teacher' && (
                               <div className="mb-3">
                                   <CustomSelect
                                       ...
                                       onChange={(val) => setSelectedSidebarClassFilter(String(val))}
                                   />
                               </div>
                           )}
                                ...
                                <>
                                     {viewMode === 'subject' ? (
                                        classSubjects.length > 0 ? (
                                            classSubjects
                                                .filter(s => s.subject?.name.toLowerCase().includes(subjectSearch.toLowerCase()))
                                                .map(subject => (
                                                    <DraggableSubject 
                                                        key={`class-subject-${subject.id}`} 
                                                        data={subject} 
                                                        isSelected={selectedClassSubject?.id === subject.id}
                                            ...
                                            <div className="flex flex-col items-center justify-center py-10 text-center px-4">
                                                ...
                                                <p className="text-xs text-gray-400 mt-1">Try selecting a class first or check your search.</p>
                                            </div>
                                        )
                                     ) : (
                                        // Teacher Mode
                                         teacherSubjects.length > 0 ? (
                                            teacherSubjects
                                                .filter(s => s.subject?.name.toLowerCase().includes(subjectSearch.toLowerCase()))
                                                .filter(s => !selectedSidebarClassFilter || 
                                                    classSubjects.find(cs => cs.subjectId === s.subjectId && String(cs.classId) === selectedSidebarClassFilter)
                                                )
                                                .map(subject => (
                                                    <DraggableSubject 
                                            ...
                                            <div className="flex flex-col items-center justify-center py-10 text-center px-4">
                                                <p className="text-sm text-gray-500">No allowed subjects found for this teacher.</p>
                                            </div>
                                        )
                                     )}
                                </>
                    ...
                    <div className="bg-white dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm min-h-[600px] flex flex-col min-w-0 w-full max-w-full">
                        ...
                        <div className="p-6 pb-0 relative">
                             <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                 <div>
                                     <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Teaching Schedule Templates</h1>
                                     <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage weekly class schedules and teacher assignments.</p>
                                 </div>
                             </div>
                        </div>
                            ...
                            <div className="flex gap-8 relative" ref={tabsRef}>
                                {tabs.map((tab) => (
                                    <button
                                        ...
                                        onClick={() => setViewMode(tab.id as 'subject' | 'teacher')}
                        ...
                        <div className="flex-1 bg-white dark:bg-transparent min-w-0 overflow-hidden flex flex-col">
                             <div className="p-6 flex-1 w-full flex flex-col overflow-hidden">
                                              ...
                                              <div className="space-y-1">
                                                  ...
                                                  <div className="relative z-50">
                                                     <SearchableAsyncSelect
                                                         ...
                                                         onChange={(_v, _l, opt) => setSelectedTeacher(opt as { label: string; value: string; subLabel?: string } | null)}
                                                     /> 
                                                  </div>
                                              </div>
                                          )}
                                      ...
                                      <div className="flex flex-wrap items-center gap-3">
                                          {(
                                              ...
                                              <>
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                try {
                                                                    ...
                                                                    if (viewMode === 'subject' && selectedViewClass) {
                                                                        const classId = selectedViewClass;
                                                                        blob = await academicService.printClassSchedule(classId);
                                                                        filename = `schedule-class-${classId}.pdf`;
                                                                    } else if (viewMode === 'teacher' && selectedTeacher) {
                                                                        const teacherId = selectedTeacher.value;
                                                                        blob = await academicService.printTeacherSchedule(teacherId);
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                try {
                                                                    ...
                                                                    if (viewMode === 'subject' && selectedViewClass) {
                                                                        const classId = selectedViewClass;
                                                                        blob = await academicService.exportExcelClassSchedule(classId);
                                                                        filename = `schedule-class-${classId}.xlsx`;
                                                                    } else if (viewMode === 'teacher' && selectedTeacher) {
                                                                        const teacherId = selectedTeacher.value;
                                                                        blob = await academicService.exportExcelTeacherSchedule(teacherId);
                                                   ...
                                                   <div className="flex gap-2">
                                                        <button
                                                            onClick={() => showError("Import Excel API not available yet")}
                                                        ...
                                                        <button
                                                            onClick={() => showError("Template download not available yet")}
                                  ...
                                  {/* Matrix */}
                                  {(viewMode === 'subject' ? selectedViewClass : selectedTeacher) ? (
                                      <div className="border border-gray-100 dark:border-white/5 rounded-xl overflow-hidden w-full max-w-full flex flex-col min-w-0 flex-1">
                                          {viewMode === 'subject' && !selectedClassSubject && selectedViewClass && (
                                               <div className="px-6 py-4 bg-gray-50/50 dark:bg-white/[0.02] border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
                                                   <div className="flex items-center gap-4">
                                                       ...
                                                       <div>
                                                           <h3 className="font-bold text-gray-900 dark:text-white">Class Schedule Summary</h3>
                                                   ...
                                                   <div className="flex items-center gap-4 min-w-[200px]">
                                                       <div className="flex-1 flex flex-col gap-1.5">
                                                           {(() => {
                                                               ...
                                                               const totalScheduled = templates.reduce((acc, t) => {
                                          ...
                                          {viewMode === 'subject' && selectedClassSubject && (
                                              <div className="px-6 py-4 bg-gray-50/50 dark:bg-white/[0.02] border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
                                                  <div className="flex items-center gap-4">
                                                      ...
                                                      <div>
                                                          <h3 className="font-bold text-gray-900 dark:text-white">{selectedClassSubject?.subject?.name}</h3>
                                                          <p className="text-xs text-gray-500">{selectedClassSubject?.class?.name} • {(selectedClassSubject?.subject as any)?.units || 0} Units Required</p>
                                                      </div>
                                                  </div>
                                                  ...
                                                  <div className="flex items-center gap-4 min-w-[200px]">
                                                      <div className="flex-1 flex flex-col gap-1.5">
                                                          <div className="flex justify-between text-xs font-medium">
                                                              <span className="text-gray-500">Planned Progress</span>
                                                              <span className={
                                                                 (templates.reduce((acc, t) => acc + (t.isActive ? t.plannedUnits : 0), 0) >= ((selectedClassSubject?.subject as any)?.units || 0))
                                                                 ? "text-green-600" 
                                                                 : "text-brand-600"
                                                              }>
                                                                 {templates.reduce((acc, t) => acc + (t.isActive ? t.plannedUnits : 0), 0)} / {(selectedClassSubject?.subject as any)?.units || 0} Units
                                                              </span>
                                                          </div>
                                                          <div className="h-2 w-full bg-gray-100 dark:bg-white/10 rounded-full overflow-hidden">
                                                              <div 
                                          ...
                                          {viewMode === 'teacher' && selectedTeacher && teacherWorkload && (
                                              <div className="px-6 py-4 bg-gray-50/50 dark:bg-white/[0.02] border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
                                                  <div className="flex items-center gap-4">
                                                      ...
                                                      <div>
                                                          <h3 className="font-bold text-gray-900 dark:text-white">{teacherWorkload.teacher?.name || selectedTeacher.label}</h3>
                                                          <p className="text-xs text-gray-500">
                                                              {teacherWorkload.academicYear?.name} • <span className={teacherWorkload.status === 'OVERLOAD' ? 'text-red-500 font-bold' : teacherWorkload.status === 'UNDERLOAD' ? 'text-yellow-600 font-medium' : 'text-green-600 font-medium'}>
                                                  ...
                                                  <div className="flex items-center gap-4 min-w-[200px]">
                                                      <div className="flex-1 flex flex-col gap-1.5">
                                                          {(() => {
                                                              ...
                                                              const activeUnits = templates.reduce((acc, t) => {
                                                                if (!t.isActive) return acc;
                                                                if (t.plannedUnits) return acc + t.plannedUnits;
                                                              ...
                                                              const targetUnits = teacherWorkload.targetUnitsPerWeek || 0;
                                                              const minUnits = teacherWorkload.minUnitsPerWeek || 0;

                                                              return (
                                                                  <>
                                                                       <div className="flex justify-between text-xs font-medium">
                                                                           <span className="text-gray-500">Workload Progress</span>
                                                                           <span className={
                                                                               activeUnits > targetUnits
                                                                               ? "text-red-600" 
                                                                               : "text-brand-600"
                                                                           }>
                                                                               {parseFloat(activeUnits.toFixed(1))} / {targetUnits} JP
                                                                           </span>
                                                                       </div>
                                                                       <div className="h-2 w-full bg-gray-100 dark:bg-white/10 rounded-full overflow-hidden">
                                                                           <div 
                                                ...
                                                <ScheduleMatrix 
                                                  ...
                                                  onDropSubject={(subject, day) => {
                                                      if ('class' in subject) {
                                                           ...
                                                           if ((viewMode as string) === 'subject') {
                                                              ...
                                                              handleOpenModal(day, undefined, subject as ClassSubject);
                                                           } else {
                                                               setDroppedSubject(null);
                                                               setFormData({
                                                                   classSubjectId: String(subject.id),
                                                                   defaultTeacherId: selectedTeacher?.value || "",
                                                                   dayOfWeek: day,
                                                                   startTime: "08:00",
                                                                   endTime: "09:30",
                                                                   plannedUnits: 0,
                                                          ...
                                                          setFormData({
                                                              classSubjectId: "",
                                                              defaultTeacherId: ts.teacher?.public_id || selectedTeacher?.value || "",
                                                              dayOfWeek: day,
                                                              startTime: "08:00",
                                                              endTime: "09:30",
                                                              plannedUnits: 0,
                                                  ...
                                                  onEditSession={(session) => {
                                                      ...
                                                      if (session.defaultTeacher) {
                                                          setTeacherOptions(prev => {
                                                              const teacherId = String(session.defaultTeacher?.id || session.defaultTeacherId);
                                                              const teacherName = session.defaultTeacher?.name || "Unknown Teacher";
                                                              if (!prev.some(o => o.value === teacherId)) {
                                                                  return [...prev, { label: teacherName, value: teacherId }];
                                                              }
                                                              return prev;
                                                      ...
                                                      setFormData({
                                                          classSubjectId: String(session.classSubjectId),
                                                          defaultTeacherId: session.defaultTeacherId ? String(session.defaultTeacherId) : "",
                                                  ...
                                                  onMoveSession={(template, newDay) => handleMoveSession(template.id, newDay, template.startTime, template.endTime)}
                                              />
                                              </div>
                                          </div>
                                      </div>
                                       ...
                                       <div className="flex flex-col items-center justify-center h-full text-center p-8 space-y-4 border border-dashed border-gray-200 dark:border-white/10 rounded-xl my-8">
                                           ...
                                           <div className="max-w-xs">
                                               <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                                                   {viewMode === 'subject' ? "No Subject Selected" : "No Teacher Selected"}
                                               </h3>
      ...
      <Modal
        ...
        footer={
           <div className="flex justify-end gap-3 w-full">
               <button onClick={() => setIsModalOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.05]">Cancel</button>
               <button type="submit" form="template-form" className="rounded-xl bg-brand-500 px-6 py-2 text-sm font-medium text-white transition-all hover:bg-brand-600 shadow-lg shadow-brand-500/20">{selectedTemplate ? "Update" : "Add Session"}</button>
           </div>
        }
      >
         <form id="template-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative group overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-white/5 transition-all duration-300 hover:shadow-xl hover:shadow-brand-500/5">
                ...
                <div className="flex items-center gap-4 relative z-10">
                    <div className="size-12 rounded-2xl flex items-center justify-center shadow-lg transform transition-transform group-hover:rotate-3 shadow-brand-500/20"
                         style={{ backgroundColor: stringToPastelColor(selectedClassSubject?.subject?.name || "Subject") }}>
                        <DocsIcon className="size-6" style={{ color: stringToDarkerColor(selectedClassSubject?.subject?.name || "Subject") }} />
                    </div>
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                             <h4 className="text-lg font-bold text-gray-900 dark:text-white truncate">
                                {droppedSubject?.subject?.name || selectedClassSubject?.subject?.name || "Select Subject"}
                            </h4>
                            {selectedTemplate && (
                                <span className="px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 dark:bg-brand-500/20 dark:text-brand-400 text-[10px] font-bold uppercase tracking-wider">Editing</span>
                            )}
                        </div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center gap-1.5 mt-0.5">
                            <TableIcon className="size-3.5" />
                            {droppedSubject ? "Manual Assignment" : (selectedClassSubject?.class?.name || "No Class Selected")}
                            {selectedClassSubject?.academicYear && (
                                <span className="text-gray-300 dark:text-white/10 mx-1">|</span>
                            )}
                            <span className="opacity-70">{selectedClassSubject?.academicYear?.name}</span>
                        </p>
                    </div>
                </div>
                ...
                {(droppedSubject || !selectedClassSubject) && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-white/5">
                         <SearchableAsyncSelect
                            ...
                            onChange={(val) => setFormData({ ...formData, classSubjectId: String(val) })}
            ...
            <div className="grid grid-cols-2 gap-5">
                ...
                <SearchableAsyncSelect
                    ...
                    onChange={(val) => setFormData({ ...formData, defaultTeacherId: String(val) })}
                    onSearch={searchTeachers}
                    options={teacherOptions}
                    isLoading={isSearchingTeachers}
                />
            ...
            <AvailabilityTimeline 
                ...
                rule={effectiveRules ? (effectiveRules[formData.dayOfWeek] || effectiveRules[formData.dayOfWeek.toUpperCase()]) : undefined}
                otherSessions={templates.filter(t => t.dayOfWeek === formData.dayOfWeek && t.id !== selectedTemplate?.id)}
                onSelectTime={(start, end) => setFormData({ ...formData, startTime: start, endTime: end })}
            />
            ...
            <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-gray-50/50 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="space-y-1">
                <Label className="text-sm font-bold text-gray-900 dark:text-white">Active Status</Label>
                <p className="text-xs text-gray-500 dark:text-gray-400">Include this session in the current schedule template calculations.</p>
              </div>
              <Switch
