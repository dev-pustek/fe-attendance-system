...
const CurriculumExplorer: React.FC = () => {
  ...
  const handleSaveLevel = async (e: React.FormEvent) => {
      ...
      const confirmed = await confirm({
          ...
          title: isEditMode ? 'Update Education Level' : 'Create Education Level',
          message: `Are you sure you want to ${isEditMode ? 'update' : 'create'} "${levelFormData.name}"?`
      });
      ...
      try {
          if (isEditMode && levelFormData.id) {
              ...
              showSuccess("Education Level created successfully");
          }
  ...
  const handleToggleLevelStatus = async (level: EducationLevel) => {
      try {
          ...
          showSuccess(`Education Level ${!level.isActive ? 'activated' : 'deactivated'}`);
      } catch (error) {
          showError(error);
      }
  };
  ...
  return (
    <>
      <PageMeta title="Curriculum Explorer | SIAPUS" description="Manage hierarchical academic structure." />
      ...
      <Modal isOpen={isLevelModalOpen} onClose={() => setIsLevelModalOpen(false)} title={isEditMode ? "Update Education Level" : "Create Education Level"} className="max-w-xl">
          <form onSubmit={handleSaveLevel} className="space-y-4 pt-4">
              <div className="space-y-1.5">
                  ...
                  <Input 
                      ...
                      placeholder="e.g. Sekolah Menengah Kejuruan" 
                      required 
                  />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-white/10 px-4 py-3 bg-gray-50/50 dark:bg-white/5">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Requires Major?</span>
                  <Switch checked={levelFormData.requiresMajor} onChange={(checked) => setLevelFormData({...levelFormData, requiresMajor: checked})} />
              </div>
              <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-white/10 px-4 py-3 bg-gray-50/50 dark:bg-white/5">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Level</span>
                  <Switch checked={levelFormData.isActive} onChange={(checked) => setLevelFormData({...levelFormData, isActive: checked})} />
              </div>
              <div className="flex justify-end gap-3 pt-6">
                  <button type="button" onClick={() => setIsLevelModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-gray-500">Cancel</button>
    ...
    <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm min-h-[600px] flex flex-col">
       ...
       <div className="p-6 border-b border-gray-100 dark:border-white/5 bg-gray-50/30 dark:bg-white/[0.01]">
            <div className="flex items-center justify-between">
                <div>
                     <p className="text-sm text-gray-500 dark:text-gray-400">Manage all education levels and their curriculum structures.</p>
                </div>
            </div>
       </div>
       ...
       <div className="p-6 bg-gray-50/50 dark:bg-black/5 flex-1 w-full box-border">
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">No Education Levels Found</h3>
                     <p className="text-gray-500 max-w-sm mt-2 mb-6">Start building your curriculum by adding an education level (e.g., High School, University).</p>
                     <button
                        onClick={onCreate}
                    ...
                    >
                        ...
                        <div className="text-center relative">
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5 font-medium max-w-[180px]">
                                Define a new target curricula for your institution
                            </p>
                        </div>
                    </div>
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                        ...
                                        <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                            {level.metrics?.programStudiesCount || 0} Programs
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                        ...
                                        <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                            {level.metrics?.majorsCount || 0} Majors
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                        ...
                                        <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                            {level.metrics?.gradesCount || 0} Grades
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                        ...
                                        <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                            {level.metrics?.subjectsCount || 0} Subjects
                                        </span>
                                    </div>
                                </div>
                            </div>
...
const LevelPanel = ({ id, name, requiresMajor, activeTab, setActiveTab, onSelectProgram }: { 
  ...
  const tabs = useMemo(() => {
    const baseTabs = [];
    if (requiresMajor) {
        baseTabs.push({ id: "programs", label: "Program Studies" });
    }
    baseTabs.push({ id: "grades", label: "Grades" });
    
    if (!requiresMajor) {
      baseTabs.push({ id: "subjects", label: "Subjects" });
    }
    return baseTabs;
  }, [requiresMajor]);
...
const ProgramStudyList = ({ levelId, onSelect }: { levelId: number | string, onSelect: (p: ProgramStudy) => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirm({
        ...
        title: isEdit ? 'Update Program' : 'Create Program',
        message: `Are you sure you want to ${isEdit ? 'update' : 'create'} "${formData.name}"?`
    });
    ...
    try {
      if (isEdit) {
          ...
          showSuccess("Program Study updated");
      } else {
          await createMutation.mutateAsync({ ...formData });
          showSuccess("Program Study added");
      }
      setIsOpen(false);
  ...
  const handleDelete = async (prog: ProgramStudy) => {
      const ok = await confirm({ variant: "delete", title: "Delete Program", message: `Delete ${prog.name}?` });
      if (ok) {
          try {
  ...
  const handleToggleStatus = async (prog: ProgramStudy) => {
      try {
          ...
          showSuccess(`${prog.name} is now ${!prog.isActive ? "Active" : "Inactive"}`);
      } catch (err) { showError(err); }
  };
  ...
  return (
    <div className="space-y-6">
            ...
            {!isLoading && (
                ...
                >
                    ...
                    <div className="text-center">
                        <span className="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">Add New Program</span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Create a new program study</span>
                    </div>
                </div>
            )}
                        ...
                        <div className="p-5 flex flex-col gap-5">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    <FolderIcon className="size-3 text-purple-500/80" />
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {p.metrics?.majorsCount || 0} Majors
                                    </span>
                                </div>
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    <DocsIcon className="size-3 text-brand-500/80" />
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {p.metrics?.subjectsCount || 0} Subjects
                                    </span>
                                </div>
                            </div>
                        </div>
      ...
      <Modal 
        ...
        title={selectedProgram ? "Update Program Study" : "Add Program Study"} 
        description="Configure program study details and status."
        className="max-w-xl"
        footer={
          <div className="flex justify-end gap-3">
             <button type="button" onClick={() => setIsOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.05]">Cancel</button>
      ...
      >
        <form id="program-form" onSubmit={handleSubmit} className="space-y-4">
           ...
           <div className="space-y-1.5">
             ...
             <Input value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Science" required />
           </div>
           <div className="space-y-1.5">
             <Label>Code</Label>
             <Input value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value})} placeholder="e.g. IPA" required />
           </div>
           ...
           {/* Status Switch (Only for Edit or default new) */}
            <div className="space-y-1.5">
                <Label>Status</Label>
                <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-white/10 px-4 py-3 bg-gray-50/50 dark:bg-white/5">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Program</span>
...
const GradeList = ({ levelId }: { levelId: number | string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirm({
        variant: isEdit ? 'update' : 'create',
        title: isEdit ? 'Update Grade' : 'Create Grade',
        message: `Are you sure you want to ${isEdit ? 'update' : 'create'} "${formData.name}"?`
    });
  ...
  const handleDelete = async (grade: Grade) => {
      const ok = await confirm({ variant: "delete", title: "Delete Grade", message: `Delete ${grade.name}?` });
      if (ok) {
          try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
           ...
           <div 
            ...
            >
                ...
                <div className="text-center">
                    <span className="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">Add New Grade</span>
                </div>
            </div>
                        ...
                        <div className="p-5 flex flex-col gap-5">
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center justify-between gap-2">
                                    <span className="text-[10px] font-bold text-brand-500/80 uppercase tracking-widest dark:text-brand-400/80">
                                        {g.educationLevel?.name || levels.find(l => l.id === g.educationLevelId)?.name || "Level"}
                                    </span>
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    ...
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {g.metrics?.classesCount || 0} Classes
                                    </span>
                                </div>
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    <DocsIcon className="size-3 text-brand-500/80" />
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {g.metrics?.subjectsCount || 0} Subjects
                                    </span>
                                </div>
                            </div>
                        </div>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={selectedGrade ? "Update Grade" : "Add Grade"} className="max-w-xl">
           <form onSubmit={handleSubmit} className="space-y-4 pt-4">
               ...
               <div className="space-y-1.5">
                 <Label>Code</Label>
                 <Input value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value})} placeholder="e.g. 10" required />
               </div>
               <div className="flex justify-end gap-3 pt-6">
                 <button type="button" onClick={() => setIsOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.05]">Cancel</button>
...
const MajorList = ({ programId, levelId, onSelect }: { 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirm({
        variant: isEdit ? 'update' : 'create',
        title: isEdit ? 'Update Major' : 'Create Major',
        message: `Are you sure you want to ${isEdit ? 'update' : 'create'} "${formData.name}"?`
    });
    ...
    try {
      if (isEdit) {
          ...
          showSuccess("Major updated");
      } else {
          await createMutation.mutateAsync({ ...formData });
          showSuccess("Major added");
      }
      setIsOpen(false);
    } catch (err) { showError(err); }
  };
  ...
  const handleDelete = async (major: Major) => {
      const ok = await confirm({ variant: "delete", title: "Delete Major", message: `Delete ${major.name}?` });
      if (ok) {
          try {
  ...
  const handleToggleStatus = async (major: Major) => {
      try {
          ...
          showSuccess(`${major.name} is now ${!major.isActive ? "Active" : "Inactive"}`);
      } catch (err) { showError(err); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
           ...
           <div 
            ...
            >
                ...
                <div className="text-center relative">
                    <span className="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">Add New Major</span>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Create a new major specialist</p>
                </div>
            </div>
                        ...
                        <div className="p-5 flex flex-col gap-5">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    <GroupIcon className="size-3 text-blue-500/80" />
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {m.metrics?.classesCount || 0} Classes
                                    </span>
                                </div>
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    <DocsIcon className="size-3 text-brand-500/80" />
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {m.metrics?.subjectsCount || 0} Subjects
                                    </span>
                                </div>
                            </div>
                        </div>
      ...
      <Modal 
        isOpen={isOpen} 
        onClose={() => setIsOpen(false)} 
        title={selectedMajor ? "Update Major" : "Add Major"} 
        description="Set up major details and active status."
        className="max-w-xl"
        footer={
           <div className="flex justify-end gap-3">
             <button type="button" onClick={() => setIsOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.05]">Cancel</button>
             <button type="submit" form="major-form" className="rounded-xl bg-brand-500 px-6 py-2 text-sm font-medium text-white transition-all hover:bg-brand-600 shadow-lg shadow-brand-500/20">
                {selectedMajor ? "Update Major" : "Save Major"}
             </button>
           </div>
        }
      >
          <form id="major-form" onSubmit={handleSubmit} className="space-y-4">
             ...
             <div className="space-y-1.5">
               ...
               <Input value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Multimedia" required />
             </div>
             <div className="space-y-1.5">
               <Label>Code</Label>
               <Input value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value})} placeholder="e.g. MMD" required />
             </div>
             ...
             <div className="space-y-1.5">
                 <Label>Status</Label>
                 <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-white/10 px-4 py-3 bg-gray-50/50 dark:bg-white/5">
                     <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Major</span>
                     <Switch checked={formData.isActive} onChange={(checked) => setFormData({...formData, isActive: checked})} />
                 </div>
             </div>
          </form>
...
const SubjectList = ({ majorId, levelId, majorName }: { majorId?: number | string, levelId?: number | string, majorName?: string }) => {
  ...
  const handleSaveInstructor = async () => {
      ...
      try {
          ...
          showSuccess(`${selectedTeacherIds.length} Instructor(s) assigned`);
  ...
  const handleRemoveInstructor = async (tsId: number | string, teacherName: string) => {
      const ok = await confirm({ 
          ...
          message: `Remove ${teacherName} from this subject?` 
      });
      if (ok) {
          try {
  ...
  const handleDelete = async (s: Subject) => {
      const ok = await confirm({ variant: "delete", title: "Delete Subject", message: `Delete ${s.name}?` });
      if (ok) {
          try {
  ...
  const handleToggleStatus = async (s: Subject) => {
      try {
          ...
          showSuccess(`${s.name} is now ${!s.isActive ? "Active" : "Inactive"}`);
      } catch (err) { showError(err); }
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex-1 space-y-1.5">
             ...
             {grades.length > 0 && (
                <div className="w-full sm:w-48 space-y-1.5">
                    <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Filter by Grade</label>
                    <CustomSelect
                        value={gradeFilter}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
           ...
           <div 
            ...
            >
                ...
                <div className="text-center relative">
                    <span className="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">Add New Subject</span>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Create a new subject entry</p>
                </div>
            </div>
                        ...
                        <div className="p-5 flex flex-col gap-5">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-2">
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50/50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                                    <GroupIcon className="size-3 text-blue-500/80" />
                                    <span className="text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                                        {s.teacherSubjects?.length || 0} Instructors
                                    </span>
                                </div>
      ...
      <Modal 
        isOpen={isOpen} 
        onClose={() => setIsOpen(false)} 
        title={selectedSubject ? "Update Subject" : "Add New Subject"} 
        description="Create or edit subject details."
        className="max-w-lg"
        footer={
           <div className="flex justify-end gap-3">
             <button type="button" onClick={() => setIsOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.05]">Cancel</button>
             <button type="submit" form="subject-form" className="rounded-xl bg-brand-500 px-6 py-2 text-sm font-medium text-white transition-all hover:bg-brand-600 shadow-lg shadow-brand-500/20">
                {selectedSubject ? "Update Subject" : "Save Subject"}
             </button>
           </div>
        }
      >
          <form id="subject-form" onSubmit={handleSubmit} className="space-y-4">
             ...
             <div className="space-y-1.5">
               ...
               <Input value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Mathematics" required />
             </div>
             <div className="space-y-1.5">
               <Label>Code</Label>
               <Input value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value})} placeholder="e.g. MATH-10" required />
             </div>
             ...
             <div className="space-y-1.5">
                 <Label>Status</Label>
                 <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-white/10 px-4 py-3 bg-gray-50/50 dark:bg-white/5">
                     <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Subject</span>
                     <Switch checked={formData.isActive} onChange={(checked) => setFormData({...formData, isActive: checked})} />
                 </div>
             </div>
          </form>
      ...
      <Modal
         ...
         title="Manage Instructors"
         description={`Assign or remove teachers for the subject "${subjectForInstructor?.name}"`}
         className="max-w-4xl"
      >
                      ...
                      <div className="space-y-4">
                          <div className="space-y-1.5">
                              ...
                              <CustomSelect
                                  ...
                                  options={teachers.map((t: User) => {
                                      ...
                                      return { 
                                          label: isAssigned ? `${t.name} (Assigned)` : t.name, 
                                          value: t.public_id 
                                      };
                                  })}
                          ...
                          {selectedTeacherIds.length > 0 ? (
                              <div className="space-y-2">
                                  <Label>Selected for Assignment</Label>
                              ...
                              <div className="py-4 text-center">
                                  <p className="text-xs text-gray-400 italic">Select teachers from the dropdown to add them to the queue</p>
                              </div>
                          )}
                          ...
                          >
                              ...
                              ) : `Assign ${selectedTeacherIds.length > 0 ? selectedTeacherIds.length : ''} Instructor(s)`}
                          </button>
                      </div>
                  </div>
              </div>
                      ...
                      {subjectForInstructor?.teacherSubjects && subjectForInstructor.teacherSubjects.length > 0 ? (
                          subjectForInstructor.teacherSubjects.map((ts: TeacherSubject) => (
                              <div key={ts.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 dark:bg-white/5 dark:border-white/10 group transition-all hover:bg-white hover:shadow-sm dark:hover:bg-white/15">
                                  ...
                                  <button
                                      type="button"
                                      onClick={() => handleRemoveInstructor(ts.id, ts.teacher?.name || "Unknown")}
                          ...
                          <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-gray-100 dark:border-white/5 rounded-2xl bg-gray-50/50 dark:bg-white/5">
                              <p className="text-xs text-gray-400 italic font-medium uppercase tracking-widest opacity-60">No instructors assigned</p>
                          </div>
                      )}
                  </div>
              </div>
