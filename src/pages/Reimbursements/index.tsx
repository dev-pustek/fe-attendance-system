...
import { useReimbursements, useReimbursementsInfinite } from '../../api/hooks/useReimbursements';
...
const Reimbursements: React.FC = () => {
  ...
  const { data: desktopData, isLoading: isDesktopLoading } = useReimbursements({
      page, limit,
  ...
  const handleDelete = async (id: string) => {
      const confirmed = await confirm({
          variant: "delete",
          title: "Hapus Klaim Biaya",
          message: "Apakah Anda yakin ingin menghapus klaim biaya ini?"
      });
      if (confirmed) {
          try {
              // TODO: Wire up actual delete mutation if backend supports it
              showSuccess("Klaim biaya berhasil dihapus");
              setSelectedIds(prev => {
              ...
              showError(error, "Gagal menghapus klaim biaya");
          }
      }
  };
  ...
  const handleBulkDelete = async () => {
      ...
      const confirmed = await confirm({
          variant: "delete",
          title: `Hapus ${count} Klaim Biaya`,
          message: `Apakah Anda yakin ingin menghapus ${count} klaim biaya yang dipilih?`
      });
      if (confirmed) {
          try {
              // TODO: Bulk delete mutation
              showSuccess(`${count} klaim biaya berhasil dihapus`);
  ...
  return (
      <>
          <PageMeta title="Klaim Biaya | SIAPUS" description="Manajemen Klaim Biaya (Reimbursements)" />
          <PageBreadcrumb pageTitle="Klaim Biaya" />
          ...
          <div className="space-y-6">
              ...
              <div className="hidden sm:flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center gap-3">
                      <div>
                          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Daftar Klaim Biaya</h1>
                          <p className="text-sm text-gray-500 dark:text-gray-400">Ajukan dan pantau status klaim biaya operasional Anda.</p>
                      </div>
                  </div>
                  <div className="flex items-center gap-3">
                      <button onClick={() => handleOpenModal()}
                          ...
                          <PlusIcon className="fill-white size-4" /> Ajukan Klaim
                      </button>
                  </div>
              </div>
                  ...
                  >
                      <div className="text-left">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                              Gunakan kriteria di bawah ini untuk memfilter daftar klaim biaya.
                          </p>
                      </div>
                  ...
                  <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-in-out ${isFilterOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                      <div className="overflow-hidden min-h-0">
                          <div className="px-5 pb-5">
                              ...
                              <div className="grid grid-cols-1 gap-5 items-end md:grid-cols-4">
              ...
              {isMobile ? (
                  <div className="space-y-3">
                      ...
                      {isLoading && items.length === 0 ? (
                          ...
                          <div className="py-12 text-center text-sm text-gray-500">Belum ada pengajuan klaim biaya.</div>
                      ) : (
                      ...
                      <Table>
                          <TableHeader className="border-b border-gray-100 bg-gray-50/60 dark:border-white/[0.05] dark:bg-white/[0.01]">
                              <TableRow>
                                  ...
                                  <TableCell isHeader className="px-4 py-3.5">
                                      <button onClick={() => handleSort("dateIncurred")} className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-brand-500 uppercase tracking-wider">
                                          Tanggal <SortIcon column="dateIncurred" />
                                      </button>
                                  </TableCell>
                                  <TableCell isHeader className="px-4 py-3.5">
                                      <button onClick={() => handleSort("description")} className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-brand-500 uppercase tracking-wider">
                                          Keterangan <SortIcon column="description" />
                                      </button>
                                  </TableCell>
                                  <TableCell isHeader className="px-4 py-3.5">
                                      <button onClick={() => handleSort("amount")} className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-brand-500 uppercase tracking-wider">
                          ...
                          <TableBody>
                              {isLoading ? (
                                  ...
                                  <TableRow>
                                      <TableCell colSpan={6} className="py-10 text-center text-gray-500">Belum ada pengajuan klaim biaya.</TableCell>
                                  </TableRow>
                              ) : (
                                  sortedItems.map(item => (
                                      <TableRow key={item.public_id} className={`group transition-colors ${selectedIds.has(item.public_id) ? "bg-brand-50/60 dark:bg-brand-500/5" : "hover:bg-gray-50/60 dark:hover:bg-white/[0.015]"}`}>
                                          ...
                                          <TableCell className="px-4 py-4 text-sm text-gray-900 dark:text-white">
                                              {formatDate(item.dateIncurred)}
                                          </TableCell>
                                          <TableCell className="px-4 py-4 text-sm text-gray-600 dark:text-gray-300 max-w-md truncate" title={item.description}>
                                              {item.description}
                                          </TableCell>
          ...
          <ReimbursementModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} selectedEntity={selectedEntity} />
