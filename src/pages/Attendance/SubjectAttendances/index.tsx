...
const SubjectAttendances: React.FC = () => {
  ...
  const { data: response, isLoading, createMutation, updateMutation, deleteMutation } = useSubjectAttendances({
  ...
  useEffect(() => {
     if (urlSessionId) {
         attendanceService.getTeachingSessions({ limit: 1 }).then(() => { 
  ...
  const handleBulkSubmit = async () => {
      ...
      try {
          ...
          showSuccess(`Successfully recorded attendance for ${selectedBulkStudents.size} students.`);
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Subject Attendance</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Track student attendance for each subject session.</p>
          </div>
          ...
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                ...
                <TableCell isHeader className="px-5 py-4 text-theme-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider text-left">Remarks</TableCell>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {isLoading ? (
                ...
                sortedAttendances.map((record: SubjectAttendance) => (
                  <TableRow key={record.id} className="group hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                    <TableCell className="px-5 py-4">
                      <div className="space-y-1">
                         ...
                         <div className="flex items-center gap-2 text-xs text-gray-500">
                           ...
                           {record.teachingSession?.startTime.substring(0, 5)} - {record.teachingSession?.endTime.substring(0, 5)}
                         </div>
                      </div>
                    </TableCell>
                    <TableCell className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900 dark:text-white text-theme-sm">{record.student?.name || "Unknown"}</p>
                    ...
                    <TableCell className="px-5 py-4">
                       <p className="text-sm text-gray-600 dark:text-gray-300 truncate max-w-[200px]" title={record.remarks || ""}>
                         {record.remarks || "-"}
                       </p>
                    </TableCell>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-3">
             ...
             <button type="submit" form="subject-attendance-form" className="rounded-xl bg-brand-500 px-6 py-2 text-sm font-medium text-white transition-all hover:bg-brand-600 shadow-lg shadow-brand-500/20">{selectedAttendance ? "Update Record" : "Save Record"}</button>
          </div>
        }
      >
          <form id="subject-attendance-form" onSubmit={handleSubmit} className="space-y-4">
                ...
                onSearch={useCallback(async (term: string) => {
                    try {
                        setIsSearchingSessions(true);
                         ...
                         setSessionOptions(res.data.map((s: TeachingSession) => ({
                            label: `${s.sessionDate} (${s.startTime})`,
                            value: String(s.id),
                            subLabel: `${s.classSubject?.class?.name} - ${s.classSubject?.subject?.name}`
                         })));
                    } catch (e) { console.error(e); }
                    finally { setIsSearchingSessions(false); }
                }, [])}
            ...
            <CustomSelect
                ...
                onChange={(val) => setFormData({ ...formData, status: String(val) as SubjectAttendance['status'] })}
                options={[
                    { label: "Present", value: "present" },
                    { label: "Absent", value: "absent" },
                    { label: "Late", value: "late" },
      ...
      <Modal
         ...
         subHeader={
              <div className="flex items-center justify-between px-6 py-4 bg-gray-50/50 dark:bg-white/[0.02]">
                   <div className="flex items-center gap-4">
                       ...
                       />
                       <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{selectedBulkStudents.size} selected</span>
                   </div>
                   <div className="flex items-center gap-2">
                       <span className="text-sm text-gray-500 whitespace-nowrap">Set All To:</span>
         ...
         footer={
             <div className="flex justify-end gap-3">
                 <button onClick={() => setIsBulkModalOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.05]">Cancel</button>
                 <button onClick={handleBulkSubmit} className="rounded-xl bg-brand-500 px-6 py-2 text-sm font-medium text-white transition-all hover:bg-brand-600 shadow-lg shadow-brand-500/20">Save Bulk Records</button>
             </div>
         }
      >
           <div className="space-y-4">
               <div className="bg-white dark:bg-transparent rounded-xl border border-gray-100 dark:border-white/5 overflow-hidden">
                    {isLoadingBulk ? (
                        ...
                        <p className="text-center py-12 text-gray-500">No students found for this class.</p>
                    ) : (
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                {bulkStudents.map(student => {
                                    ...
                                    return (
                                        <tr key={sid} className={`transition-colors ${isSelected ? 'bg-brand-50/30 dark:bg-brand-500/5' : 'hover:bg-gray-50 dark:hover:bg-white/[0.02]'}`}>
                                            <td className="p-4">
                                                <Checkbox 
                                                   checked={isSelected}
                                                   onChange={(c) => {
                                                ...
                                                <div className="relative w-32">
                                                    <select
                                                        ...
                                                        className={`h-9 w-full appearance-none rounded-lg border bg-white px-3 py-1 text-sm outline-none transition-all focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 dark:bg-white/[0.05] dark:text-white ${
                                                            isSelected ? 'border-brand-300 dark:border-brand-500/30' : 'border-gray-200 dark:border-white/10'
                                                        }`}
                                                        disabled={!isSelected}
                                                    >
                                                       <option value="present">Present</option>
