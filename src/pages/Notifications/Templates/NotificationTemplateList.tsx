...
export default function NotificationTemplateList() {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.checked) {
          setSelectedIds(sortedTemplates.map(t => t.id));
      } else {
          setSelectedIds([]);
      }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = await confirm({
        ...
        message: `Are you sure you want to permanently delete ${selectedIds.length} selected templates? This action cannot be undone.`,
        confirmText: `Delete ${selectedIds.length} Templates`
    });

    if (confirmed) {
        try {
            const promises = selectedIds.map(id => notificationService.deleteTemplate(id));
            await Promise.all(promises);
            showSuccess(`Successfully removed ${selectedIds.length} templates.`);
            setSelectedIds([]);
            fetchTemplates();
        } catch (error) {
            showError(error, "Failed to remove some templates");
        }
    }
  };
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center justify-between p-4 bg-brand-50 border border-brand-100 rounded-2xl dark:bg-brand-500/10 dark:border-brand-500/20 animate-in slide-in-from-top-2 duration-300">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-semibold text-brand-700 dark:text-brand-400">Templates Selected</p>
            </div>
                    ...
                    <TableRow>
                        <TableCell isHeader className="px-5 py-4 w-12">
                            <input 
                                ...
                                checked={sortedTemplates.length > 0 && selectedIds.length === sortedTemplates.length}
